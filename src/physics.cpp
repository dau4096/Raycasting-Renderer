...
bool quickIntersect(glm::vec3 pointA, glm::vec3 pointB, structs::Wall wall, float* distSQ) {
	...
	if ((std::min(wall.start.z, wall.end.z) - 1e-4f <= z) && (z <= std::max(wall.start.z, wall.end.z) + 1e-4f)) {
		if (distSQ != nullptr) {
...
bool isInsideVP(glm::vec2 point2D, structs::Visplane plane) {
...
bool didHitSwitch(size_t switchIdx) {
	...
	for (size_t idx=0; idx<validWalls; idx++) {
		structs::Wall thisWall = physicsData->wallData.at(idx);
...
//Walls
void applyWallDirectionalMovement(structs::Wall* wall, float speed, bool enabled) {
...
void applyWallNormalMovement(structs::Wall* wall, float speed, bool enabled) {
	glm::vec2 wallDir2D = glm::normalize(glm::vec2(wall->start) - glm::vec2(wall->end));
...
void applyWallZMovement(structs::Wall* wall, float speed, bool enabled) {
	if (abs(wall->data) < constants::SPECIAL_MOVE_SPEED_SLOW) { //Movement is not significant enough to carry out.
		return;
...
void applyWallRotation(structs::Wall* wall, float speed, bool enabled) {
	if (abs(wall->data) < constants::SPECIAL_MOVE_SPEED_SLOW) { //Movement is not significant enough to carry out.
		return;
...
//Visplanes
void applyVisplaneXMovement(structs::Visplane* vPlane, float speed, bool enabled) {
...
void applyVisplaneYMovement(structs::Visplane* vPlane, float speed, bool enabled) {
	if (abs(vPlane->data) < constants::SPECIAL_MOVE_SPEED_SLOW) { //Movement is not significant enough to carry out.
		return;
...
void applyVisplaneZMovement(structs::Visplane* vPlane, float speed, bool enabled) {
	if (abs(vPlane->data) < constants::SPECIAL_MOVE_SPEED_SLOW) { //Movement is not significant enough to carry out.
		return;
...
void playerMovement() {
	structs::Player playerCopy = player;
	...
	for (const structs::Sprite& sprite : physicsData->spriteData) {
...
void updatePhysicsObjects() {
	std::vector<structs::Sprite> newSpriteData;
	for (structs::Sprite thisSprite : physicsData->spriteData) {
		if ((thisSprite.type == SPR_INVALID) || (thisSprite.type == SPR_DECO) || (thisSprite.type == SPR_LIGHT)) {
			newSpriteData.push_back(thisSprite);
			continue; /* Non-physics sprites. */
		}
		structs::Sprite spriteCopy = thisSprite;
		thisSprite.prevPosition = thisSprite.position;


		bool touchingFloor = false;
		for (structs::Visplane thisPlane : physicsData->visplaneData) {
			touchingFloor |= isInsideVP(glm::vec2(thisSprite.position), thisPlane) && ((thisSprite.position.z - thisPlane.height) <= (thisSprite.height / 2.0f));
		}


		if (touchingFloor) {
			thisSprite.velocity *= constants::FLOOR_FRICT_COEFF;
		} else {
			float gAccel = thisSprite.mass * stageData.gravity / constants::PHYSICS_FREQUENCY;
			thisSprite.velocity.z -= gAccel;
			thisSprite.velocity *= constants::AIR_FRICT_COEFF;
		}
		thisSprite.position += thisSprite.velocity;

		bool isAlive = true;
		if (thisSprite.type == SPR_PARTICLE) {
			thisSprite.transparency -= 1.0f / constants::PARTICLE_LIFETIME_FRAMES;
			isAlive = thisSprite.transparency > 0.0f;
		}

		if (!isAlive) {
			validSprites--;
			continue; /* Remove from dataset. */
		}

		if (isVec3NaN(thisSprite.position) || isVec3NaN(thisSprite.velocity)) {
			//Something went very wrong.
			newSpriteData.push_back(spriteCopy);
		} else {
			newSpriteData.push_back(thisSprite); //Add to the new set.
		}
	}
	physicsData->spriteData = newSpriteData;
	{
		std::lock_guard<std::mutex> lock(stateSwapMutex);
		graphicsData->spriteData = newSpriteData;
	}
}
...
void updateSpecials(bool interactKey) {
	...
	for (size_t vIndex=0; vIndex<validVisplanes; vIndex++) {
		...
		switch(vPlane.type) {
			...
			case V_HURT: {
				if (planeTouch && (tickNumber % static_cast<int>(constants::PHYSICS_FREQUENCY)) == 0) {
					//Damage player by its data value once per second.
					utils::hurtPlayer(vPlane.data);
				}
				break;
			}
			...
			case V_TELEPORT: {
				if (planeTouch && (vPlane.internal->first <= 0.0f)) {
					...
					structs::Visplane partner = physicsData->visplaneData.at(partnerIndex);
					...
					glm::vec3 particleStartPoint = player.position - glm::vec3(0.0f, 0.0f, player.height / 3.0f);
					...
					glm::vec3 particleEndPoint = player.position - glm::vec3(0.0f, 0.0f, player.height / 3.0f);
					particles::createParticleLine(particleStartPoint, particleEndPoint, P_ENERGY, 64);
